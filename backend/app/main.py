...
from .database import (
    ...
    create_collection,
    assign_note_to_collection,
    get_collection_by_id,
    get_smart_study_queue,
    get_note_revisions,
    get_note_revision_by_id,
    restore_note_from_revision,
    get_mastery_map_by_user,
...
from .schemas import (
    ...
    NoteResponse,
    NoteListResponse,
    NoteCollectionCreate,
    NoteCollectionResponse,
    AssignNoteCollectionRequest,
    SmartStudyQueueResponse,
    ...
    TranscriptToNoteRequest,
    TranscriptToNoteResponse,
    GraphResponse,
    SuccessResponse,
    GraphNode,
    NoteConnection,
...
@app.get("/notes/search", response_model=NoteListResponse)
async def search_notes(
    ...
    """Universal notes search by title/content/tags with optional collection filter."""
    await check_rate_limit(request, str(user_id))

    notes = search_notes_by_user(db, user_id, q, collection_id, skip, limit)
    return NoteListResponse(
        notes=[
            NoteResponse(
                id=note.id,
...
@app.get("/collections", response_model=List[NoteCollectionResponse])
async def list_collections(
    ...
    return [
        NoteCollectionResponse(
            ...
            note_count=len(collection.notes or []),
        )
        for collection in collections
    ]


@app.post("/collections", response_model=NoteCollectionResponse)
async def create_collection_endpoint(
    payload: NoteCollectionCreate,
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    await check_rate_limit(request, str(user_id))
    collection = create_collection(db, user_id, payload.name)
...
@app.put("/notes/{note_id}/collection", response_model=NoteResponse)
async def assign_note_collection_endpoint(
    note_id: uuid.UUID,
    payload: AssignNoteCollectionRequest,
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    ...
    if payload.collection_id:
        collection = get_collection_by_id(db, payload.collection_id, user_id)
        if not collection:
            raise HTTPException(status_code=404, detail="Collection not found")

    updated = assign_note_to_collection(db, note, payload.collection_id)
    return NoteResponse(
        id=updated.id,
        title=updated.title,
...
async def update_existing_note(
    ...
    return NoteResponse(
        ...
        collection_id=updated_note.collection_id,
    )
...
@app.get("/notes/{note_id}/related", response_model=List[BacklinkResponse])
async def fetch_related_notes(
    note_id: uuid.UUID,
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Return related notes combining manual links and similarity edges."""
    await check_rate_limit(request, str(user_id))
    seed = get_note_by_id(db, note_id, user_id)
    if not seed:
        raise HTTPException(status_code=404, detail="Note not found")

    notes = get_notes_by_user(db, user_id, 0, 1000)
    note_map = {n.id: n for n in notes}
    similarities = get_similarities_for_notes(db, [n.id for n in notes])
    links = get_links_for_user_notes(db, user_id)

    related_ids = set()
    for link in links:
        if link.from_note_id == note_id:
            related_ids.add(link.to_note_id)
        if link.to_note_id == note_id:
            related_ids.add(link.from_note_id)

    for sim in similarities:
        if sim.note_a_id == note_id and sim.similarity >= 150:
            related_ids.add(sim.note_b_id)
        elif sim.note_b_id == note_id and sim.similarity >= 150:
            related_ids.add(sim.note_a_id)

    related_notes = [note_map[rid] for rid in related_ids if rid in note_map]
    related_notes.sort(key=lambda n: n.updated_at or n.created_at, reverse=True)
...
@app.get("/notes/{note_id}/revisions", response_model=List[NoteRevisionResponse])
async def list_note_revisions(
    ...
    if not note:
        raise HTTPException(status_code=404, detail="Note not found")
    revisions = get_note_revisions(db, note_id, user_id, limit)
    return [
        NoteRevisionResponse(
            id=revision.id,
...
@app.post("/notes/{note_id}/revisions/restore", response_model=NoteResponse)
async def restore_note_revision(
    ...
    revision = get_note_revision_by_id(db, payload.revision_id, user_id)
    if not revision or revision.note_id != note_id:
        raise HTTPException(status_code=404, detail="Revision not found")

    updated = restore_note_from_revision(db, note, revision)
    try:
        record_event(
            db,
            user_id,
            ...
            metadata={"action": "revision_restore", "revision_id": str(revision.id)},
        )
    except Exception:
        pass
...
@app.post("/ai/transcript-to-note", response_model=TranscriptToNoteResponse)
async def transcript_to_note(
    request_data: TranscriptToNoteRequest,
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
):
    """Convert transcript text into structured note content using Gemini (with fallback)."""
    await check_ai_rate_limit(request, str(user_id))

    try:
        result = await generate_note_from_transcript(
            request_data.transcript, request_data.preferred_title
        )

        html_content = result.get("content_html", "")
        plain_word_count = len(re.sub(r"<[^>]+>", " ", html_content).split())

        return TranscriptToNoteResponse(
            title=result.get("title", "Lecture Notes"),
            content_html=html_content,
            summary=result.get("summary"),
            word_count=plain_word_count,
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to generate note from transcript: {str(e)}",
        )
...
async def summarize_note(
    ...
    # Robust summary generation (provider failure should not break note flow)
    summary = await summarize_content(note.content)
    if not summary:
        summary = " ".join((note.content or "").split()[:120]).strip()

    try:
        updated_note = update_note(db, note, summary=summary)
    except Exception:
        # If persisting summary fails, return the computed summary without failing request
        return NoteResponse(
            id=note.id,
            title=note.title,
            content=note.content,
...
@app.get("/study/queue", response_model=SmartStudyQueueResponse)
async def get_smart_queue_endpoint(
    request: Request,
    flashcards_limit: int = 20,
    notes_limit: int = 5,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Student-first study queue: due flashcards + weakest note areas."""
    await check_rate_limit(request, str(user_id))

    queue = get_smart_study_queue(db, user_id, flashcards_limit, notes_limit)
    due_flashcards = [
        FlashcardResponse(
            id=flashcard.id,
            question=flashcard.question,
        ...
        for flashcard in queue["due_flashcards"]
    ]
    return SmartStudyQueueResponse(
        due_flashcards=due_flashcards,
        weak_notes=queue["weak_notes"],
        due_count=queue["due_count"],
        weak_count=queue["weak_count"],
    )


@app.post("/study/exam/start", response_model=ExamSessionStartResponse)
async def start_exam_session(
    payload: ExamSessionStartRequest,
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Start an exam session from currently due flashcards."""
    await check_rate_limit(request, str(user_id))
    due_flashcards = get_due_flashcards(db, user_id, payload.limit)
    cards = [
        FlashcardResponse(
            id=flashcard.id,
            question=flashcard.question,
...
@app.get("/study/mastery-map", response_model=List[MasteryMapItem])
async def get_mastery_map(
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Return note-level mastery for topic map UIs."""
    await check_rate_limit(request, str(user_id))
    data = get_mastery_map_by_user(db, user_id)
    return [MasteryMapItem(**item) for item in data]


@app.get("/study/reminders/preview", response_model=List[ReminderPreviewItem])
async def get_reminders_preview(
    request: Request,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Preview reminder feed used by in-app/email reminder surfaces."""
    await check_rate_limit(request, str(user_id))
    due_cards = get_due_flashcards(db, user_id, 50)
    weak_notes = get_mastery_map_by_user(db, user_id)[:3]
    current_streak, _ = compute_streaks(db, user_id)

    reminders: List[ReminderPreviewItem] = []
    if due_cards:
        reminders.append(
            ReminderPreviewItem(
                type="due_flashcards",
                title="Flashcards due now",
                detail=f"You have {len(due_cards)} flashcards ready for review.",
            )
        )
    else:
        reminders.append(
            ReminderPreviewItem(
                ...
                detail="No due cards right now. Do one quick review session to stay sharp.",
            )
        )
    ...
    reminders.append(
        ReminderPreviewItem(
            ...
            detail=f"You are on a {current_streak}-day study streak.",
        )
    )

    for item in weak_notes:
        reminders.append(
            ReminderPreviewItem(
                type="weak_topic",
                title=f"Revisit {item['title']}",
                detail=f"Current mastery is {item['avg_mastery']}%.",
            )
        )
...
@app.get("/study/weekly-digest/preview", response_model=WeeklyDigestResponse)
async def get_weekly_digest_preview(
    request: Request,
    days: int = 7,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: Session = Depends(get_db),
):
    """Preview digest payload for UI and future email delivery."""
    await check_rate_limit(request, str(user_id))
    now = datetime.now(timezone.utc)
    since = now - timedelta(days=days)
    type_counts = get_event_type_counts(db, user_id, since, now)
    due_cards = get_due_flashcards(db, user_id, 200)
    reminders = await get_reminders_preview(
        request=request,
        user_id=user_id,
        db=db,
    )

    notes_created = type_counts.get("NOTE_CREATED", 0)
    flashcards_reviewed = type_counts.get("FLASHCARD_REVIEWED", 0)

    return WeeklyDigestResponse(
        period_days=days,
        notes_created=notes_created,
        flashcards_reviewed=flashcards_reviewed,
        due_flashcards=len(due_cards),
        reminders=reminders,
    )
