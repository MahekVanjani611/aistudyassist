...
class User(Base):
    ...
    collections = relationship(
        "NoteCollection", back_populates="user", cascade="all, delete-orphan"
    )
...
class Note(Base):
    ...
    collection_id = Column(
        UUID(as_uuid=True), ForeignKey("note_collections.id"), nullable=True, index=True
    )

    # Relationships
    user = relationship("User", back_populates="notes")
    collection = relationship("NoteCollection", back_populates="notes")
    flashcards = relationship(
...
class NoteRevision(Base):
    """Revision snapshots for note history and restore."""
...
def create_tables():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Add collection_id if missing
            conn.execute(
                sql_text(
                    "ALTER TABLE notes ADD COLUMN IF NOT EXISTS collection_id UUID NULL"
                )
            )
            # Collections table (dev convenience; use Alembic in prod)
            conn.execute(
                sql_text(
                    """
                    ...
                    CREATE INDEX IF NOT EXISTS idx_note_collections_user ON note_collections(user_id);
            ...
            conn.execute(
                sql_text(
                    ...
                    CREATE INDEX IF NOT EXISTS idx_note_revisions_user ON note_revisions(user_id, created_at DESC);
                    """
                )
            )
...
def search_notes_by_user(
    ...
    """Universal note search across title/content/tags with optional collection filter."""
    notes_query = db.query(Note).filter(Note.user_id == user_id)

    if collection_id:
        notes_query = notes_query.filter(Note.collection_id == collection_id)
    ...
    if query and query.strip():
        ...
        notes_query = notes_query.filter(
            or_(
                ...
                Note.tags.any(query.strip()),
            )
        )

    return (
        notes_query.order_by(Note.updated_at.desc()).offset(skip).limit(limit).all()
    )
...
def get_collections_by_user(db: Session, user_id: uuid.UUID) -> list[NoteCollection]:
    return (
        db.query(NoteCollection)
        .filter(NoteCollection.user_id == user_id)
        .order_by(NoteCollection.name.asc())
        .all()
    )


def create_collection(db: Session, user_id: uuid.UUID, name: str) -> NoteCollection:
    collection = NoteCollection(user_id=user_id, name=name.strip())
    db.add(collection)
    db.commit()
...
def get_smart_study_queue(
    db: Session, user_id: uuid.UUID, flashcards_limit: int = 20, notes_limit: int = 5
) -> Dict[str, Any]:
    """Build a student-first queue from due flashcards + weak note areas."""
    due_cards = get_due_flashcards(db, user_id, flashcards_limit)

    weak_note_rows = (
        db.query(
            Note.id.label("note_id"),
            Note.title.label("title"),
            func.coalesce(func.avg(Flashcard.mastery_level), 0).label("avg_mastery"),
            func.count(Flashcard.id).label("flashcard_count"),
            func.max(Flashcard.last_reviewed).label("last_reviewed"),
        )
        .outerjoin(Flashcard, Flashcard.note_id == Note.id)
        .filter(Note.user_id == user_id)
        .group_by(Note.id, Note.title)
        .order_by(sql_text("avg_mastery ASC"), sql_text("last_reviewed NULLS FIRST"))
        .limit(notes_limit)
        .all()
    )
    ...
    weak_notes = [
        {
            ...
            "title": row.title,
            "avg_mastery": int(row.avg_mastery or 0),
            "flashcard_count": int(row.flashcard_count or 0),
            "last_reviewed": row.last_reviewed,
        }
        for row in weak_note_rows
    ]
...
def create_note_revision(db: Session, note: Note) -> NoteRevision:
...
def get_note_revisions(
    db: Session, note_id: uuid.UUID, user_id: uuid.UUID, limit: int = 20
) -> list[NoteRevision]:
    return (
        db.query(NoteRevision)
        .filter(NoteRevision.note_id == note_id, NoteRevision.user_id == user_id)
...
def get_note_revision_by_id(
    db: Session, revision_id: uuid.UUID, user_id: uuid.UUID
) -> Optional[NoteRevision]:
    return (
        db.query(NoteRevision)
        .filter(NoteRevision.id == revision_id, NoteRevision.user_id == user_id)
        .first()
    )


def restore_note_from_revision(
    db: Session, note: Note, revision: NoteRevision
) -> Note:
    create_note_revision(db, note)
    note.title = revision.title
    note.content = revision.content
    note.summary = revision.summary
...
def get_mastery_map_by_user(db: Session, user_id: uuid.UUID):
    """Aggregate note-level mastery for topic heatmap views."""
    rows = (
        db.query(
            Note.id.label("note_id"),
            Note.title.label("title"),
            Note.tags.label("tags"),
            func.coalesce(func.avg(Flashcard.mastery_level), 0).label("avg_mastery"),
            func.count(Flashcard.id).label("flashcard_count"),
        )
        .outerjoin(Flashcard, Flashcard.note_id == Note.id)
        .filter(Note.user_id == user_id)
        .group_by(Note.id, Note.title, Note.tags)
        .order_by(sql_text("avg_mastery ASC"))
        .all()
    )
    return [
        {
...
def get_event_type_counts(
    ...
    rows = (
        db.query(Event.event_type, _func.count().label("count"))
        .filter(
            Event.user_id == user_id,
            Event.occurred_at >= from_date_utc,
