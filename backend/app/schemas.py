...
class NoteCollectionCreate(BaseSchema):
    name: str
    ...
    def validate_name(cls, v):
        ...
        if len(v.strip()) > 120:
            raise ValueError("Collection name cannot exceed 120 characters")
        return v.strip()


class NoteCollectionResponse(BaseSchema):
    id: uuid.UUID
    name: str
    created_at: datetime
    updated_at: datetime
...
class AssignNoteCollectionRequest(BaseSchema):
    collection_id: Optional[uuid.UUID] = None


class WeakNoteItem(BaseSchema):
    note_id: uuid.UUID
    title: str
    avg_mastery: int
    flashcard_count: int
...
class SmartStudyQueueResponse(BaseSchema):
    due_flashcards: List[Dict[str, Any]]
    weak_notes: List[WeakNoteItem]
    due_count: int
    weak_count: int


class ExamSessionStartRequest(BaseSchema):
...
class ExamSessionStartResponse(BaseSchema):
    duration_minutes: int
    flashcards: List[Dict[str, Any]]
    started_at: datetime


class NoteRevisionResponse(BaseSchema):
    id: uuid.UUID
    note_id: uuid.UUID
    title: str
    content: str
...
class NoteRevisionRestoreRequest(BaseSchema):
    revision_id: uuid.UUID


class MasteryMapItem(BaseSchema):
    note_id: uuid.UUID
    title: str
    tags: Optional[List[str]] = []
    avg_mastery: int
...
class ReminderPreviewItem(BaseSchema):
    type: str
    title: str
    detail: str


class WeeklyDigestResponse(BaseSchema):
    period_days: int
    notes_created: int
...
class TranscriptToNoteRequest(BaseSchema):
    transcript: str
    ...
    def validate_transcript(cls, v):
        ...
        if len(v) > 200000:
            raise ValueError("Transcript too long for processing")
        return v.strip()


class TranscriptToNoteResponse(BaseSchema):
    title: str
    content_html: str
    summary: Optional[str] = None
    word_count: int
