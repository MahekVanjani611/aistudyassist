...
import { api, type ActivityDayCount, type EventItem, type ProfileSummary, type MasteryMapItem } from '@/lib/api'
...
export default function ProfilePage() {
  ...
  const [masteryMap, setMasteryMap] = useState<MasteryMapItem[]>([])
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const map = await api.getMasteryMap()
        setMasteryMap(map)
      } catch {}
    }
    run()
      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-8">
          <Card className="bg-white dark:bg-[#141820] border-gray-200 dark:border-[#232a36]">
            ...
            <CardContent>
              {masteryMap.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">No mastery data yet. Review flashcards to build your map.</p>
              ) : (
                <div className="space-y-3">
                  {masteryMap.slice(0, 12).map((item) => {
                    const colorClass = item.avg_mastery >= 80
                      ? 'bg-green-500'
                      : item.avg_mastery >= 50
                        ? 'bg-yellow-500'
                        : 'bg-red-500'
                    return (
                      <div key={item.note_id} className="space-y-1">
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-900 dark:text-gray-100 truncate pr-3">{item.title}</span>
                          <span className="text-gray-600 dark:text-gray-300">{item.avg_mastery}%</span>
                        </div>
                        <div className="h-2 rounded bg-gray-200 dark:bg-[#0f1115] overflow-hidden">
                          <div className={`h-2 ${colorClass}`} style={{ width: `${Math.max(2, item.avg_mastery)}%` }} />
                        </div>
                      </div>
                    )
                  })}
