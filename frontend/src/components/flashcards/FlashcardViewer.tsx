...
interface FlashcardViewerProps {
  ...
  onBack?: () => void
  autoStartExamToken?: number
}

export function FlashcardViewer({ noteId, note, onFlashcardCreated, autoStartExamToken }: FlashcardViewerProps) {
  ...
  useEffect(() => {
    if (!autoStartExamToken) return
    startExamMode()
  }, [autoStartExamToken])

  useEffect(() => {
    if (!examMode || examRemainingSeconds <= 0) return
  ...
  }, [examMode, examRemainingSeconds])

  const startExamMode = async () => {
    try {
      const exam = await api.startExamSession(15, 20)
      setExamFlashcards(exam.flashcards)
      setExamRemainingSeconds(exam.duration_minutes * 60)
      setExamMode(true)
      setCurrentIndex(0)
      setShowAnswer(false)
      setUserAnswer('')
      ...
      console.error('Failed to start exam mode:', error)
    }
  }
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            size="sm"
            onClick={examMode ? () => setExamMode(false) : startExamMode}
            className={examMode ? 'bg-orange-600 text-white' : ''}
          >
            {examMode ? 'Exit Exam' : 'Start Exam'}
          </Button>
          {examMode && (
            <div className="text-sm text-orange-600 dark:text-orange-400 font-medium">
              Time left: {String(Math.floor(examRemainingSeconds / 60)).padStart(2, '0')}:{String(examRemainingSeconds % 60).padStart(2, '0')}
            </div>
          )}
