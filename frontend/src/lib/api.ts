...
export interface SmartStudyQueue {
  due_flashcards: Flashcard[]
  weak_notes: WeakNote[]
  due_count: number
  weak_count: number
}
...
export interface TranscriptToNoteResponse {
  title: string
  content_html: string
  summary?: string
  word_count: number
...
class APIClient {
  ...
  async searchNotes(query?: string, collectionId?: string, skip: number = 0, limit: number = 100): Promise<{ notes: Note[]; total: number; page: number; per_page: number }> {
  ...
  async createCollection(name: string): Promise<NoteCollection> {
    return this.request('/collections', {
      method: 'POST',
  ...
  async assignNoteToCollection(noteId: string, collectionId: string | null): Promise<Note> {
    return this.request(`/notes/${noteId}/collection`, {
      method: 'PUT',
      body: JSON.stringify({ collection_id: collectionId }),
    })
  }

  async getSmartStudyQueue(flashcardsLimit: number = 20, notesLimit: number = 5): Promise<SmartStudyQueue> {
    return this.request(`/study/queue?flashcards_limit=${flashcardsLimit}&notes_limit=${notesLimit}`)
  }

  async startExamSession(durationMinutes: number = 15, limit: number = 20): Promise<ExamSessionStartResponse> {
    return this.request('/study/exam/start', {
      method: 'POST',
      body: JSON.stringify({ duration_minutes: durationMinutes, limit }),
    })
  }
  ...
  async getRelatedNotes(noteId: string): Promise<Array<{ note_id: string; title: string; excerpt?: string; created_at: string }>> {
    return this.request(`/notes/${noteId}/related`)
  }

  async getNoteRevisions(noteId: string, limit: number = 20): Promise<NoteRevision[]> {
    return this.request(`/notes/${noteId}/revisions?limit=${limit}`)
  }

  async restoreNoteRevision(noteId: string, revisionId: string): Promise<Note> {
    return this.request(`/notes/${noteId}/revisions/restore`, {
      method: 'POST',
  ...
  async transcriptToNote(transcript: string, preferredTitle?: string): Promise<TranscriptToNoteResponse> {
    return this.request('/ai/transcript-to-note', {
      method: 'POST',
      body: JSON.stringify({
        transcript,
  ...
  async getMasteryMap(): Promise<MasteryMapItem[]> {
    return this.request('/study/mastery-map')
  }

  async getReminderPreview(): Promise<ReminderPreviewItem[]> {
    return this.request('/study/reminders/preview')
  }

  async getWeeklyDigestPreview(days: number = 7): Promise<WeeklyDigestPreview> {
    return this.request(`/study/weekly-digest/preview?days=${days}`)
  }
